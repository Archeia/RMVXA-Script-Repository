...
WINDOW_ORDER = [:master,:bgm,:se,:resolution,:switch,:message_se,:autosave]
#Basic options are:
...
module OPT_STR
  ...
  SWITCH_ID          = 5
  SWITCH             = ""
  SWITCH_HELP        = "Any unsaved progress will be lost"
  SWITCH_COMMAND     = ["Change Resolution","Off"]
end
...
class Window_OCommand < Window_Selectable
  ...
  def draw_item(index)
    rect = item_rect(index)
    draw_text(rect, ["Apply","Cancel"][index],1)
  end
end
