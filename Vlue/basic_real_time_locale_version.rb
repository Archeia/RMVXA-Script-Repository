#Basic Real Time (System Time) Locale Version
#----------#
...
RT_MONTHS = ["janvier","fevrier","mars","avril","mai","juin","juillet",
              "aout","septembre","octobre","novembre","decembre"]
RT_DAYS = ["lundi","mardi","mercredi","jeudi","vendridi","samedi","dimanche"]

module RTime
  #Functions, explained above#
  def self.fulltime
    return Time.now.strftime(month(true) + ", " + day(true) + " %-d %Y %l:%M")
  end
  ...
  def self.month(name = false, abbr = false)
    ...
    if abbr then return Time.now.strftime("%b") else return RT_MONTHS[month-1] end
  end
  def self.day(name = false, abbr = false)
    return Time.now.day if name == false
    if abbr then return Time.now.strftime("%a") else return RT_DAYS[dayweek-1] end
  end
  def self.dayweek
