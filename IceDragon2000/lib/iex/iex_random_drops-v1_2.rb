...
#
# (DD/MM/YYYY)
#  09/12/2010 - V1.0  Finished Script.
#  09/14/2010 - V1.1  Added 3rd Mode - Steady Rate
#  08/26/2011 - V1.2  Edited for the IEX Recall
#
#------------------------------------------------------------------------------#
#==============================================================================#
# ** KNOWN ISSUES
...
# ** Math
#==============================================================================# 
module Math ; def self.min_rand(min = 0, max = 1) ; return rand(max-min) + min end end
...
class RPG::Enemy
  ...
  #--------------------------------------------------------------------------#
  # * new-method :iex_rand_drop_cache
  #--------------------------------------------------------------------------#     
  def iex_rand_drop_cache
    ...
    when IEX::REGEXP::RAND_DROP::MORE_ITEMS
      @more_drops.push([0, $1.to_i, $2.to_i, $3.to_i])
    when IEX::REGEXP::RAND_DROP::MORE_ARMORS
      @more_drops.push([2, $1.to_i, $2.to_i, $3.to_i])
    when IEX::REGEXP::RAND_DROP::MORE_WEAPONS
      @more_drops.push([1, $1.to_i, $2.to_i, $3.to_i])  
...
class Game_Enemy < Game_Battler
  ...
  # * new-method :lucky_rate_add
  #--------------------------------------------------------------------------#    
  def lucky_rate_add() ; return enemy.lucky_rate_add ; end
  
  #--------------------------------------------------------------------------#
  # * new-method :gold
  #--------------------------------------------------------------------------#    
  def gold()
    prep_gold_drop() if @gold_amount.nil?() ; return @gold_amount
  end
  
  #--------------------------------------------------------------------------#
  # * new-method :more_drops
  #--------------------------------------------------------------------------#    
  def more_drops()
    result = []
    for dro in enemy.get_more_drops
      case dro[0]
      when 0 ; result.push([$data_items[dro[1]], dro[2], dro[3]])
      when 1 ; result.push([$data_weapons[dro[1]], dro[2], dro[3]])
      when 2 ; result.push([$data_armors[dro[1]], dro[2], dro[3]])
      end  
    end  
    return result
  end
...
class Game_Troop < Game_Unit
  ...
  def initialize( *args, &block )
    iex_lucky_rate_initialize( *args, &block )
  ...
  def setup( *args, &block )
    iex_lucky_rate_setup( *args, &block )
    setup_lucky_fight()
  end
  
  #--------------------------------------------------------------------------#
  # * new-method :setup_lucky_fight
  #--------------------------------------------------------------------------# 
  def setup_lucky_fight()
    @lucky_rate = IEX::RAND_DROP::LUCKY_DROP_RATE
