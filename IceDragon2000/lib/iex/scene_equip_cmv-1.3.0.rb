...
#------------------------------------------------------------------------------#
#
#  10/10/2010 V1.0  Finished Script
#  10/26/2010 V1.1  Fixed skin problem
#  01/06/2011 V1.2  Quick fix your color issue
...
class Window_Equip < Window_Selectable
  ...
  def refresh
    ...
    if @actor.two_swords_style
      self.contents.draw_text(text_x, WLH * 0, 92, WLH, Vocab::weapon1)
      self.contents.draw_text(text_x, WLH * 1, 92, WLH, Vocab::weapon2)
      if IEX::COSMETIC_EQUIP::USE_CATERGORY_ICONS
        draw_icon(cat_icons[:weapon], text_x - 24, WLH * 0)
        draw_icon(cat_icons[:weapon], text_x - 24, WLH * 1)
      end
    else
      self.contents.draw_text(text_x, WLH * 0, 92, WLH, Vocab::weapon)
      self.contents.draw_text(text_x, WLH * 1, 92, WLH, Vocab::armor1)
      if IEX::COSMETIC_EQUIP::USE_CATERGORY_ICONS
        draw_icon(cat_icons[:weapon], text_x - 24, WLH * 0)
        draw_icon(cat_icons[:shield], text_x - 24, WLH * 1)
      end
    end
    self.contents.draw_text(text_x, WLH * 2, 92, WLH, Vocab::armor2)
    self.contents.draw_text(text_x, WLH * 3, 92, WLH, Vocab::armor3)
    self.contents.draw_text(text_x, WLH * 4, 92, WLH, Vocab::armor4)
    if IEX::COSMETIC_EQUIP::USE_CATERGORY_ICONS
        draw_icon(cat_icons[:head], text_x - 24, WLH * 2)
        draw_icon(cat_icons[:armor], text_x - 24, WLH * 3)
        draw_icon(cat_icons[:accessory], text_x - 24, WLH * 4)
     end
    off_x_sub = 12
    self.contents.font.size = 18
    draw_item_name(@data[0], text_x + (92 - off_x_sub), WLH * 0)
    draw_item_name(@data[1], text_x + (92 - off_x_sub), WLH * 1)
    draw_item_name(@data[2], text_x + (92 - off_x_sub), WLH * 2)
    draw_item_name(@data[3], text_x + (92 - off_x_sub), WLH * 3)
    draw_item_name(@data[4], text_x + (92 - off_x_sub), WLH * 4)
  end
...
class Scene_Equip < Scene_Base
  ...
  # * Start processing
  #--------------------------------------------------------------------------
  alias :iex_equip_cosmet_se_start :start unless $@
  def start(*args)
    iex_equip_cosmet_se_start(*args)
  ...
  #--------------------------------------------------------------------------
  alias iex_equip_cosmet_se_terminate terminate unless $@
  def terminate(*args)
    iex_equip_cosmet_se_terminate(*args)
    @help_window = nil
    @equip_window = nil
    @status_window = nil
  end
