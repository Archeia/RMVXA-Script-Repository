class REI::BattlerUnit
  ...
  def atk_range
    @atk_range ||= RPG::BaseItem::Range.new(MkRange::RANGE_DIAMOND,1,0)
    weapon, = weapons
    weapon ? weapon.atk_range : @atk_range
  end
  def atk_effect_range
    @atk_effect_range ||= RPG::BaseItem::Range.new(MkRange::RANGE_DIAMOND,0,0)
    weapon, = weapons
    weapon ? weapon.effect_range : @atk_effect_range
  end
  def attack_skill?(item)
    Ex_Database.skill?(item) && item.id == attack_skill_id
  end
  def get_range(item)
    return RPG::BaseItem::Range.new(MkRange::RANGE_DIAMOND,0,0) unless item
  ...
  def item_performance item
    if Ex_Database.weapon? item
      a = mk_params_for(item)
      n = a[2] + a[4] + a.inject(&:+)
    elsif Ex_Database.armor? item
      a = mk_params_for(item)
      n = a[3] + a[5] + a.inject(&:+)
    else
      n = item.performance
    end
    Integer(n * (Ex_Database.ex_equip_item?(item) ? 1.2 : 1.0))
  end
  def mk_temp_self
