module REI
class EntityBattler < EntityBase

  include ::Mixin::EffectConstants
  ...
  def initialize
    ...
    @actions = []
    @speed = 0
    @result = Game::ActionResult.new(self)
    @last_target_index = 0
    @guarding = false
  ...
  def make_actions
    ...
    @actions = Array.new(make_action_times) { Game::Action.new(self) }
  end
  #--------------------------------------------------------------------------
  # * Determine Action Speed
  #--------------------------------------------------------------------------
  def make_speed
    @speed = @actions.map {|action| action.speed }.min || 0
  end
  #--------------------------------------------------------------------------
  # * Get Current Action
  #--------------------------------------------------------------------------
  ...
  def force_action(skill_id, target_index)
    clear_actions
    action = Game::Action.new(self, true)
  ...
  def make_damage_value(user, item)
    value = item.damage.eval(user, self, $game.variables)
  ...
  def item_global_effect_apply(effect)
    if effect.code == EFFECT_COMMON_EVENT
      $game.temp.reserve_common_event(effect.data_id)
    end
  end
  #--------------------------------------------------------------------------
  ...
  def item_test(user, item)
    ...
    return true if $game.party.in_battle
  ...
  def regenerate_hp
    damage = -(mhp * hrg).to_i
    perform_map_damage_effect if $game.party.in_battle && damage > 0
